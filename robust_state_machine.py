        ...
        while True:
            try:
                ...
                if "timeout" in self.transition_matrix.get(self.current_state, {}):
                    ...
                    logging.error(f"No timeout transition defined for state '{self.current_state}' in state machine '{self.name}'")
...
# Main async function that loads and runs two state machines
async def main():
    ...
    # Load two state machines from the available files
    ...
    machine1 = StateMachine("elevator_transitions", os.path.join(state_machine_directory, file1), queue1, queue2, guard_functions)
    machine2 = StateMachine("vending_machine_transitions", os.path.join(state_machine_directory, file2), queue2, queue1, guard_functions)

    await queue1.put("start")  
    await queue2.put("start")  

    events = ["up", "stop", "door_open", "door_close", "down", "emergency_trigger", "reset"]
    for e in events:
        await queue1.put(e)

    events = ["insert_money", "select_item", "item_dispensed", "out_of_service", "repair"]
    for e in events:
        await queue2.put(e)
